			...
			<div class="dp_main">
				...
				<div class="dp_right" style="width:280px;">
					<div style="padding:2px;margin:0px 2px;font-weight:bold;font-size:14px"><?php
					 ...
					 ?>
					   <label class="tutorSearchResult-name-disc2" style="padding-left:10px;" ><?php echo $html->link($findname, array('controller'=>'members', 'action'=>'cause', $urlName), array('title' => $result['Cause']['userMeta']['cause_name'],'target' => '_blank' )); ?></label>
					</div>
					...
					<div class="dp_info">
                    	<ul>
							...
							<li>
								...
								<input type="text" name="data[Member][grant]" style="width:40px;font-size:10px;border:1px solid #CCC;padding:1px" maxlength="3" value="<?php echo $result['CauseTutor']['grant']; ?>" /> %
								
								<input type="hidden" name="data[Member][default]" value="<?php echo $result['CauseTutor']['grant']; ?>" />
								
								<input type="hidden" name="data[Member][ctId]" value="<?php echo $result['CauseTutor']['id']; ?>" />
								</span>
								<span style="margin-left:20px;color:blue;cursor:pointer;" class="grantChange">Change</span>
                                
                                <a href="<?php echo HTTP_ROOT.'members/deletetutorscause/'.$result['CauseTutor']['id']?>"><span style="margin-left:20px;color:blue;cursor:pointer;">Remove</span></a>
