	...
	<tr class="odd">
		...
		<td class="col-first" style="font-size:14px;color:#F2AC47;padding-right:10px;">
			<?php if (strlen($np['Notice']['notice_head'])>60)
			{
				...
				echo $html->link($notice_headName, array('controller'=>'notices', 'action'=>'notice_edit', base64_encode(convert_uuencode($np['Notice']['id'])),'admin' => true), array('title' => $np['Notice']['notice_head']));
		...
		<td class="row-nav">
			<?php echo $html->link('', array('controller'=>'notices', 'action'=>'notice_add', $np['Notice']['id'],'admin' => true), array("title" => "Edit NOTICE",'class'=>'table-edit-link')); ?>
