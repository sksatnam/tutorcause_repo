		...
		<div class="logo">
			...
			if($session->read('Member.memberid')){
				if($session->read('Member.group_id')==6 && $session->read('Member.active')=='1') {
					echo $html->link($html->image('frontend/logonew.png',array('width'=>200,'height'=>85)),array('controller'=>'members', 'action'=>'cause_dashboard'),array('escape' => false));
				}
            ...
            <div class="navigation">
				...
				<?php	} ?>
                        ...
                        if($this->Session->read('Member.group_id')==8 && $session->read('Member.active')=='1')
							...
							{
							echo $html->link('Help a Non-Profit', HTTP_ROOT.'members/find_cause'); 
							}
					    ?>	
						</li>
