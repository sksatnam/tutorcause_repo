...
$requesturlcause = '/members/non_profit_amount_raised/'.$getBalance['Member']['id'];
$causeamount = $this->requestAction($requesturlcause);
...
<?php	echo $this->Session->flash(); ?>
    ...
    <?php  echo $this->element('frontend/causeProfile'); ?>
        <div class="public_profile_cointainer_IInd" style="width:710px">
			...
			<li class="active"><a href="#description">Non-Profit Profile</a></li>
		...
		<div id="description" class="content" style="width:670px">
            ...
            <div class="profileContent" >
				...
				<li class="li2"><?php 
				if(!empty($getBalance['userMeta']['cause_name']))
				{
				...
				<li class="li2">
					<?php 
					if(!empty($getBalance['userMeta']['biography']))
					{
						echo $getBalance['userMeta']['biography'];	
					}
					else
					{
						echo 'N/A';
        ...
        <h2 style="color:#219eda; font-size:32px; width:670px; word-wrap:break-word; "><?php echo $getBalance['userMeta']['cause_name'];?> Tutor's</h2>
		  ...
		  foreach($filtertutor1 as $ft){
              ...
              <div class="tutorSearchResultCause" <?php if($j==$cntfilter){
			  ?>
              style="border-bottom:none;"	  
              <?php      
			  }
			  ?>>	<!--tutorSearchResult-->
                       <div class="profile-pic">	<!--tutorSearchResult-img-->
                        ...
                        if(!empty($ft['Member']['showImage']))
                        {
                        
                        if(!empty($ft['Member']['image_name']))
						{
                        echo $html->image("members/thumb/".$ft['Member']['image_name'],array('height'=> '58px', 'width' => '57px'));
                        } 
                        else {
                               ...
                               if(count($ft['TutEvent']))
                               {
                                ?>   
                                   <div class="tutorSearchResult-nameCause-disc2">	<!--tutorSearchResult-nameCause-disc2-->
                                    <label><span style="color:#096;"><b>Available</b></span> (     <?php if($this->Session->read('Member.memberid'))
											{
												if($this->Session->read('Member.group_id')==8)
												{
											?>
												 <a href="<?php echo HTTP_ROOT.'members/profiletutoravail/'.$ft['Member']['id']; ?>" style="color:color:#339;">see availability</a>
												 <?php
												 }
												 else
												 {
												 ?>
												 <a href="javascript:void(0);" onclick="dialogmsg2();return false;" style="color:#339;" >see availability</a>
											...
											<a href="javascript:void(0);" onclick="dialogmsg1();return false;" style="color:#339;" >see availability</a>
											<?php
											}
											?>)</label>
                                        ...
                                        foreach($ft['TutCourse'] as $memtutcourse )
                                            {
                                                ...
                                                echo $memtutcourse['course_id'].'<b>($ '.$memtutcourse['rate'].' / Hour)</b>'.',<br> ';
												/*if($i%2==0)
												{
												echo '<br>';
												}*/
                        ...
                        <div class="tutorSearchResult-personalInfo2">	<!--tutorSearchResult-personalInfo-->
                            <ul>
                                <?php if($ft['userMeta']['fb_allow']==1 && !empty($ft['Member']['facebookId'])) { ?>
									...
									<?php if($this->Session->read('Member.memberid'))
											{
												...
												{
											?>
												 <a href="<?php echo HTTP_ROOT.'Members/book_tutor_time/'.$ft['Member']['id']; ?>">Book Now</a>
												 <?php
												 }
												 else
												 {
