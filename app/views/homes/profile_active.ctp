 <?php 
 if($redirect == 1)
 {
	 $id = $this->Session->read('Member.id');
	 
	 $url = HTTP_ROOT.'members/checkuserstep/'.$id;
?>	 
	 <meta http-equiv='refresh' content='5; url=<?php echo $url;?>'>
 <?php    
 }
 ?>
 <div class="middleContainer">
 <?php 	echo $this->Session->flash(); ?>
	 <div class="classifiedheading">
	 <div style="float:left; width:916px; border:1px solid #ccc; border-radius:5px; -moz-border-radius:5px;  padding: 0 0 0 20px;">
		<h3><?php echo $statdatas['Page']['name'];?></h3>
		<div style="clear: both;"></div>
		<div class="faqsx" style="width:900px;font-size:12px;">
			<div class="tinymcestaticdata">
            
            	<?php echo $statdatas['Page']['body'];?>	
				<br>
			
			</div>
		</div>
	</div>
	</div>
</div>

