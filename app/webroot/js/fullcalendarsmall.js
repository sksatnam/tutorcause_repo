...
currentView.sizeDirty=false;currentView.eventsDirty=false;updateEvents(forceEventRender);elementOuterWidth=element.outerWidth();$('#calendar0').val(formatDate(currentView.start,'dd-MMM-yyyy',""));$('#calendar5').val(formatDate(currentView.start,'dd-MMM-yyyy',""));$('#calendar6').val(formatDate(addDays(currentView.start,6),'dd-MMM-yyyy',""));header.updateTitle(currentView.title);var today=new Date();if(today>=currentView.start&&today<currentView.end){header.disableButton('today');}else{header.enableButton('today');}
...
var prevButton;$.each(this.split(','),function(j,buttonName){if(buttonName=='title'){e.append("<span class='fc-header-title'><h3>&nbsp;</h3></span>");if(prevButton){prevButton.addClass(tm+'-corner-right');}
prevButton=null;}else{var buttonClick;if(calendar[buttonName]){buttonClick=calendar[buttonName];}
else if(fcViews[buttonName]){buttonClick=function(){button.removeClass(tm+'-state-hover');calendar.changeView(buttonName);if(buttonName=='agendaWeek'){$("#selectrepeat option[value='daily']").removeAttr('selected','selected');$("#selectrepeat option[value='weekly']").attr('selected','selected');$('.daily').hide();$('.weekly').show();}else{$("#selectrepeat option[value='weekly']").removeAttr('selected','selected');$("#selectrepeat option[value='daily']").attr('selected','selected');$('.daily').show();$('.weekly').hide();}};}
...
return e;}
function updateTitle(html){element.find('h3').html(html);}
