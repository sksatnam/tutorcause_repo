...
class MembersController extends AppController {
	...
	var $uses = array('Member','State','userMeta','Group', 'School', 'Course', 'TutCourse','TutEvent','Page','CauseSchool','UserImage','CauseTutor','TutMessage','Privilege','TutRating','PaymentHistory','TutorWithdrawal','TutorToCause','AddFund','CauseWithdrawal','EmailTemplate');
		...
		if ($userdata) {
			if (!empty($this->data)) {
				$this->Member->updateAll(array(
					'Member.pwd' => "'" . $this->data['Member']['pwd'] . "'",
	...
	function admin_add($id = NULL) {
		...
		if (!empty($this->data)) {
			$data['Member']['pwd']       = $this->data['Member']['pwd'];
			...
			if ($lastId) {
				...
				$userMeta['userMeta']['country']   = $this->data['userMeta']['country'];
	...
	function admin_delete($id = NULL) {
		...
		if ($this->RequestHandler->isAjax() && !$id) {
			...
			if ($delete) {
				...
				//	$image = $this->userImage->deleteAll(array('userImage.user_id' => $id));
			}
			if ($meta) {
				echo "deleted";
			}
	...
	function checkfacebooklogin() {
		...
		$facebook = new Facebook(array(
			'appId' => '154931561242507',
			'secret' => '597a90a9b0f1d41750dec2f37fd368d0'
		));
	...
	function index($id = NULL) {
		...
		// use id 32 for student
		
		// use id 26 for tutor
		
		$this->Session->write('Member.memberid', 26);
		...
		if ($user) {
			try {
				...
				if (!$Checkfbid) {
					...
					if ($this->Session->read('Member.active')=='1') {
						$this->redirect(array(
						'controller' => 'homes',
						'action' => 'block_user'
						));
	...
	function tutor_paypal_detial() {
		...
		if (isset($this->data)) {
			if ($this->data['Member']['payment'] == 'Paypal') {
				$tutorpaypal['Member']['paypalEmail'] = $this->data['Member']['paypalEmail'];
			}
			
			if ($this->data['Member']['payment'] == 'Mailed Check') {
				$tutorpaypal['Member']['mailedCheck'] = $this->data['Member']['mailedCheck'];
			}
			
			$tutorpaypal['Member']['paymentFrequency'] = $this->data['Member']['paymentFrequency'];
			...
			$tutorpaypal['Member']['id'] = $memberdata['Member']['id'];
			...
			*/
			...
			if ($this->Member->save($tutorpaypal)) {
	...
	function checkuserstep($fbid = null) {
		...
		if ($usertype == '6') {
			if ($userdata['userMeta']['fname'] != "" && $userdata['userMeta']['lname'] != "" && $userdata['Member']['email'] != "" && $userdata['userMeta']['contact'] != "" && $userdata['userMeta']['address'] != "" && $userdata['userMeta']['city'] != "" && $userdata['userMeta']['state'] != "" && $userdata['userMeta']['zip'] != "") {
			...
			if ($userdata['userMeta']['fname'] != "" && $userdata['userMeta']['lname'] != "" && $userdata['Member']['email'] != "" && $userdata['userMeta']['contact'] != "" && $userdata['userMeta']['address'] != "" && $userdata['userMeta']['city'] != "" && $userdata['userMeta']['state'] != "" && $userdata['userMeta']['zip'] != "") {
				if ($userdata['Member']['paymentFrequency'] != "" && ($userdata['Member']['paypalEmail'] != "" || $userdata['Member']['mailedCheck'] != "")) {
					...
					if ($step3) {
						...
						if ($step4) {
							$this->update_balance($this->Session->read('Member.memberid'));
							$this->redirect(array(
								'controller' => 'members',
								'action' => 'tutor_dashboard'
							));
...
*/      
	...
	*/
	...
	function save_request($id = NULL) {
		...
		$this->Session->setFlash('Your request has been send to tutor.');
		$this->redirect(array(
			'controller' => 'members',
			'action' => 'cause_dashboard'
		));
	...
	function tutoravail($id = null) {
		...
		$tutevent = $this->TutEvent->find('all', array(
			'conditions' => array(
				...
				//	'TutEvent.start_date >' => date('Y-m-d',time())
			)
		));
				...
				if (strcmp($res, "VERIFIED") == 0) {
					if ($_POST['payment_status'] == 'Completed') {
						...
						//	$headers .= 'BCc: promatics.jaswant@gmail.com' . "\r\n";
							...
							$subject       = 'Student Paying confirmation';
							$message       = $this->stdcnfm_email_template($paymentid);
	...
	function paying_session($paymentid = NULL ) {
		...
		if (isset($this->data)) {
			...
			$balance    = $balanceRow['Member']['balance'];
			if ($balance >= $amount) {
	...
	function update_balance($memberId) {
		Configure::write('debug', 0);
		$getAmount = $this->PaymentHistory->find('first', array(
			'fields' => 'SUM(PaymentHistory.tutor_rate_per_hour*PaymentHistory.tutoring_hours) AS amount',
			'conditions' => array(
				'PaymentHistory.booked_end_time <' => date('Y-m-d H:i:s',time()-(2 * 24 * 60 * 60)),
				'PaymentHistory.session_status' => 'Paided',
				'PaymentHistory.paypal_status' => 'complete',
		...
		if ($getAmount[0]['amount'] > 0) {
			if ($this->PaymentHistory->updateAll(array(
				...
				$this->Member->updateAll(array(
					'Member.creditable_balance' => ('Member.creditable_balance+' . $getAmount[0]['amount']),
					'Member.balance' => ('Member.balance-' . $getAmount[0]['amount'])
				), array(
					'Member.id' => $memberId
				));
	...
	function tutor_element() {
		...
		/*	$withdrawalreq = $this->TutorWithdrawal->find('first',
		array(
		'conditions'=>array('TutorWithdrawal.tutor_id' => $this->Session->read('Member.memberid'),
	...
	function paying_option($paymentid) {
		...
		if (isset($this->data)) {
			...
			$balance = $this->data['Member']['balance'];
			if ($method == "credit") {
				if ($balance >= $price) {
	...
	function verify_email_template($parentname,$amount,$verifyCode){
		Configure::write('debug', 0);
		$template = "<div>
			<p>Dear student</p>
				...
				if(isset($fundInfo['AddFund']['id'])){
					if($fundInfo['AddFund']['approval_status'] == 'Pending'){
						if($this->AddFund->updateAll(
							...
							$this->Member->updateAll(
								array(
									'Member.balance' => 'Member.balance+'.$fundInfo['AddFund']['amount']
								),
								array(
									'Member.id'=>$this->Session->read('Member.memberid')
								)
