...
class PaymentHistoriesController extends AppController {
	...
	var $uses = array('School', 'Member', 'State', 'userMeta', 'Group', 'Course', 'TutCourse', 'Privilege', 'TutEvent', 'PaymentHistory', 'TutorWithdrawal', 'TutorToCause', 'CauseWithdrawal', 'AddFund','Charge','CauseTutor','Wage','Api','ParentStudent','ParentFund','StudentFund');
	...
	function admin_non_profit_grant($id = NULL) {
		$this->layout = 'admin';
		Configure::write('debug', 0);
	...
	function admin_non_profit_withdrawal() {
		$this->layout = 'admin';
		Configure::write('debug', 0);
		...
		*/
		...
		if ($this->RequestHandler->isAjax()) {
			...
			$this->AutoRender = false;
			$this->viewPath   = 'elements' . DS . 'adminElements' . DS . 'payments';
			$this->render('non_profit_withdrawal');
		}
		
	}
	
	function admin_non_profit_money($id = NULL) {
		$this->layout = 'admin';
		Configure::write('debug', 0);
	...
	function admin_approve_non_profit_withdrawal() {
		$this->layout = 'admin';
		Configure::write('debug', 0);
	...
	/*	echo '<pre>';
		...
		*/
		...
		if (!$this->RequestHandler->isAjax() && !isset($this->data)) {
			$this->Session->delete('parentview');
		}
		if (isset($this->data)) {
			
				$this->Session->write('parentview.dateFrom', $this->data['PaymentHistory']['dateFrom']);
				$this->Session->write('parentview.dateTo', $this->data['PaymentHistory']['dateTo']);
				$this->Session->write('parentview.perpage', $this->data['PaymentHistory']['perpage']);
				
				$this->data['PaymentHistory']['dateFrom'] = $this->Session->read('parentview.dateFrom');
				$this->data['PaymentHistory']['dateTo']   = $this->Session->read('parentview.dateTo');
				$this->data['PaymentHistory']['perpage']   = $this->Session->read('parentview.perpage');
		...
		if (strlen($this->Session->read('parentview.dateFrom')) > 0) {
			$this->data['PaymentHistory']['dateFrom'] = $this->Session->read('parentview.dateFrom');
		} else {
			$this->data['PaymentHistory']['dateFrom'] = '';
		...
		$conditions = array();
		$conditions = array_merge($conditions, array(
			'ParentFund.payment_status' => 'complete'
		));
		
		if (isset($this->data['PaymentHistory']['dateFrom'])) {
			$conditions = array_merge($conditions, array(
				'ParentFund.created >=' => $this->data['PaymentHistory']['dateFrom']
			));
			
		}
		if (isset($this->data['PaymentHistory']['dateTo'])) {
			$conditions = array_merge($conditions, array(
				'ParentFund.created <=' => $this->data['PaymentHistory']['dateTo']
			));
			
		}
		if (strlen($this->Session->read('parentview.perpage')) > 0) {
			$this->data['PaymentHistory']['perpage'] = $this->Session->read('parentview.perpage');
		} else {
			$this->data['PaymentHistory']['perpage'] = 10;
		}
		$this->paginate['ParentFund'] = array(
			'conditions' => $conditions,
			'limit' => $this->data['PaymentHistory']['perpage']
		);
		$payments = $this->paginate('ParentFund');
	...
	function admin_parent_add_fund()
	{
		...
		if (isset($this->data['PaymentHistory']['dateFrom'])) {
			$conditions = array_merge($conditions, array(
				'StudentFund.created >=' => $this->data['PaymentHistory']['dateFrom']
			));
			
		}
		if (isset($this->data['PaymentHistory']['dateTo'])) {
			$conditions = array_merge($conditions, array(
				'StudentFund.created <=' => $this->data['PaymentHistory']['dateTo']
			));
		...
		$this->paginate['StudentFund'] = array(
			'conditions' => $conditions,
			'limit' => $this->data['PaymentHistory']['perpage']
		);
		$payments = $this->paginate('StudentFund');
