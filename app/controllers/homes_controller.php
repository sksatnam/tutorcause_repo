...
class HomesController extends AppController {
	...
	var $uses = array('Member','State','userMeta','UserImage','Group', 'School', 'Course', 'TutCourse','TutEvent','Page','ContactUsMessage','AddFund','CauseSchool','TutMessage','Wage','Timezone');
	...
	/*	if(!$this->Session->read('Member.memberid'))
		...
		$statdatas= $this->Page->find('all',array('conditions'=> array('Page.id' => '42')));
		$this->set("statdatas",$statdatas);
	...
	function tutor_application() {
		
		$this->layout="frontend";
	
		$statdatas= $this->Page->find('all',array('conditions'=> array('Page.id' => '51')));
		$this->set("statdatas",$statdatas);
	...
	function parents()
	{
		...
		$this->set('securelink', $securelink);
		...
		if ($this->RequestHandler->isAjax()) {
			...
			if ($user) {
				...
				if (!empty($getMemberData)) {
					...
					$this->Session->write('Member.timezone', $getMemberData['Member']['timezone']);
					$this->Session->write('Member.offset', $getMemberData['Member']['offset']);
					$this->Session->write('Member.created', $getMemberData['Member']['created']);
...
//		$this->set('emailStatus','');
	...
	function savemember()
	{
		...
		if (isset($this->data)) {
			...
			/*	$schoolname= $this->School->find('first',array('conditions'=>array('School.id'=>$this->data['Member']['school_id'])));
				$memberdata['Member']['timezone'] = $schoolname['School']['timezone'];
				$memberdata['Member']['offset'] = $schoolname['School']['offset'];
	...
	/*		
			...
			$schoolname= $this->School->find('first',array('conditions'=>array('School.id'=>$this->data['CauseSchool']['school_id'][0])));
			$memberdata['Member']['timezone'] = $schoolname['School']['timezone'];
			$memberdata['Member']['offset'] = $schoolname['School']['offset'];	
		...
		/*	echo 'ok'.'<pre>';
			...
			/*	$userMeta['userMeta']['fb_allow'] = $this->data['userMeta']['fb_allow'];
				...
				if (!empty($getMemberData)) {
					...
					$this->Session->write('Member.email', $getMemberData['Member']['email']);
					$this->Session->write('Member.create_time', $getMemberData['Member']['created']);
					$this->Session->write('Member.memberid', $getMemberData['Member']['id']);
					$this->Session->write('Member.active', $getMemberData['Member']['active']);
					$this->Session->write('Member.group_id', $getMemberData['Member']['group_id']);
					$this->Session->write('Member.timezone', $getMemberData['Member']['timezone']);
	...
	function forget_password()
	{
		...
		if($this->data)
		{
			...
			if($memberData)
			{
				...
				$this->Session->setFlash('New password will be sent to your Email- Id');
...
*/      
	...
	*/
	...
	function addcourse($id = NULL) {
		...
		{
			$this->Session->write('courses',$this->data['TutCourse']['course_id'][0]);
			$this->Session->write('rates',$this->data['TutCourse']['rate'][0]);
			$this->Session->setFlash('There are duplicate courses.');
			$this->Session->write('prices',$this->data['Tuttrates']['marks']);
...
*/		
		...
		$this->redirect(array('controller'=>'homes', 'action'=>'tutor_application'));
	...
	function send_first_msg($id=NULL)
	{
		...
		{
			$data['TutMessage']['subject']  = 'Welcome '.$userName;
			$data['TutMessage']['message']  = "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec placerat rutrum leo id tempus. Vestibulum euismod magna sed nulla blandit pretium. Morbi sollicitudin orci enim. ";
		}
	...
	function sendgrid_unsubscribe($email = NULL)
	{
		...
		$email = trim($email);
		$url = "http://sendgrid.com/api/unsubscribes.add.xml?api_user=tutorcause&api_key=fp9K81G16R1X84F&email=$email";
	...
	/*	echo '<pre>';
		...
		echo $msg;*/
			...
			if($msg=='success')
			{
				$this->redirect(array('controller'=>'homes','action'=>'unsubscribed_sendgrid'));
			}
			else
			{
				$this->redirect(array('controller'=>'members','action'=>'error'));
			}
	...
	function unsubscribed_sendgrid()
	{
		
		Configure::write('debug', 0);
		$this->layout="frontend";
		$statdatas= $this->Page->find('all',array('conditions'=> array('Page.id' => '53')));
		$this->set("statdatas",$statdatas);
