...
class MembersController extends AppController {
	...
	/*	if($_POST)
		...
		*/	
			...
			if($this->data['Member']['group_id']==8)
			{
...
//		$this->set('emailStatus','');
	...
	/*	$this->Session->write('Member.memberid', 8);
	...
	/*	if($this->Session->read('Member.memberid'))
		...
		if($id)
		{
			$tutevent     = $this->TutEvent->find('all', array(
				...
				'fields' => array('TutEvent.id','TutEvent.tutor_id','TutEvent.title','TutEvent.start_date','TutEvent.end_date'),
			...
			if(count($tutevent))
			{
				  $startdate = $tutevent[0]['TutEvent']['start_date'];
				  $day = date("j", strtotime($startdate));
				  $month = date("n", strtotime($startdate));
				  $month = $month - 1;
				  $year = date("Y", strtotime($startdate));
...
/*	function update_balance() {
	...
	function cause_element() {
		...
		$amountraised = $this->cause_amount_raised($memberid);
	...
	/*	if(!$this->Session->read('Member.memberid'))
		...
		if($this->Session->read('Member.memberid'))
		{
			...
			if($studentdata['Member']['stripeid']!='')
			{
...
//		$id = convert_uudecode(base64_decode($id));
	...
	function tutor_give_cause($tutorid = NULL, $causeid = NULL )
	{
	...
	function deletetutorscause($id = NULL)
	{
		...
		$causedata =  $this->CauseTutor->find('first',array('conditions'=>
											  array('CauseTutor.id' => $id),
											  'recursive' => 2
											  )
								);
...
/*				$schooldata = $this->School->find('first',array('conditions'=>
		...
		if($_REQUEST['causeid'])
		{
			...
			$watchcause =  $this->userMeta->find('first',array('conditions'=>array('userMeta.id'=>$id),
																  )
								);
		...
		if(count($filtercause)==0)
			{
				$this->Session->setFlash('No Cause found.');
			}
...
*/
	...
	function update_amount_raised()
	{
		...
		if($this->data['cause']['goal'])
		{
		
			$this->userMeta->updateAll(array(
					'userMeta.goal' => "'" . $this->data['cause']['goal'] . "'"
				), array(
					'userMeta.member_id' => $this->Session->read('Member.memberid')
				) 
			);
			
			$this->Session->setFlash('Cause goal updated.');
	...
	//	Stripe::setApiKey("hzKG6oNLVXaJpipg5j2AWqv0gl90MQZi");
		...
		$addfund['AddFund']['amount']          = $card_type;
		$addfund['AddFund']['payment_status']  = 'pending';
		$addfund['AddFund']['approval_status'] = 'Pending';
		$this->AddFund->create();
		$this->AddFund->save($addfund);
...
//		$stdemail = $_POST['studentemail'];
	...
	function getdate_event() {
		...
		$startDate = date("d-M-Y",strtotime($eventData['TutEvent']['start_date']));
		
		$startTime = date("h:i A",strtotime($eventData['TutEvent']['start_date']));
		
		$endTime = date("h:i A",strtotime($eventData['TutEvent']['end_date']));
